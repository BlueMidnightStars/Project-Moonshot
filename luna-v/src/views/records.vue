      ...
      <header>
        <div class="history_top">
          <nav class="history_top_item">
            <router-link to="/not" class="today">今日</router-link>
            <img src="./../assets/image/history-frame.png">
            <router-link to="/records" class="history">历史</router-link>
            <router-link to="/mine" href="" class="mine">我的OKR</router-link>
          </nav>
        </div>
      </header>
      <div class="container" v-for='(item,id) in dom' :key='id'>
        <p class="time_top">{{item.updated_at}}</p>
        <div class="container_first">
          <h1>今日三件事</h1>
          <div class="container_first_item">
            <div :class="move(im.status)" v-for='(im,index) in item.todos' :key='index'>
				<span></span>
				<p class="container_first_item_a-title">{{im.value}}</p>
				<p class="container_first_item_a-desc" @click='check(item.id)'>|  OKR</p>
            </div>
          </div>
        </div>
        ...
        <div class="container_4th">
          <dl>
            <dt>幸福五元素</dt>
            <dd v-for='(im,index) in item.happness' :key='index'>{{mood(im)}}</dd>
          </dl>
        </div>
      </div>
    </div>
    <div class="state-section" v-if='hide'>
    	<div class="state-container">
    		...
    		<div class="state-desc">
    			<p class="state-desc-text" v-for='(im,index) in state' :key='index' >{{im.keyresults[0]}}:{{im.objectives}}</p>
    		</div>
...
export default {
  ...
  methods:{
  	move:function(key){
  		if (key == 1) {
  			return 'container_first_item_a active';
  		}else{
  			return 'container_first_item_a';
  		}
  	},
  	mood:function(id){
  		if (id == 1) {
  ...
  .major{
    ...
    .container{
      ...
      .container_first{
        ...
        .container_first_item{
          width: 590px/@ppr;
          list-style: none;
          margin: 0 auto;
            .container_first_item_a{
            	width: 100%;
	            height: 100px/@ppr;
	            box-sizing: border-box;
		...
		.active{
			.container_first_item_a-title{
          	...
          	span{
          		background-image:url('./../assets/image/save-frame.png');
